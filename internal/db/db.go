...
func Open(path string, readOnly bool) (*DB, error) {
	...
	if !readOnly {
		...
		if err != nil {
			pool.Close()
			return nil, fmt.Errorf("taking migration connection: %w", err)
		}
...
func (d *DB) Checkpoint() error {
	return d.withConn(context.Background(), func(conn *sqlite.Conn) error {
		return sqlitex.ExecuteTransient(conn, "PRAGMA wal_checkpoint(TRUNCATE)", nil)
	})
}
...
func dbURI(path string) string {
	...
	return fmt.Sprintf("file:calvin-%d?mode=memory&cache=shared", memoryDBCounter.Add(1))
}

func (d *DB) withConn(ctx context.Context, fn func(*sqlite.Conn) error) error {
...
func (d *DB) UpsertEvent(e calendar.Event, syncGen int64) error {
	return d.withConn(context.Background(), func(conn *sqlite.Conn) error {
		return upsertEvent(conn, e, syncGen)
	})
}

func upsertEvent(conn *sqlite.Conn, e calendar.Event, syncGen int64) error {
...
func (d *DB) GetSyncToken(calendarID string) (string, error) {
	var token string
	err := d.withConn(context.Background(), func(conn *sqlite.Conn) error {
		return sqlitex.ExecuteTransient(conn, `SELECT token FROM sync_state WHERE calendar_id = ?`, &sqlitex.ExecOptions{
			Args: []any{calendarID},
			ResultFunc: func(stmt *sqlite.Stmt) error {
...
func (d *DB) SetSyncToken(calendarID, token string) error {
	return d.withConn(context.Background(), func(conn *sqlite.Conn) error {
		return setSyncToken(conn, calendarID, token)
	})
}

func setSyncToken(conn *sqlite.Conn, calendarID, token string) error {
	return sqlitex.ExecuteTransient(conn, `
...
func (d *DB) GetSyncGeneration() (int64, error) {
	var gen int64
	err := d.withConn(context.Background(), func(conn *sqlite.Conn) error {
		return sqlitex.ExecuteTransient(conn, `SELECT COALESCE(MAX(sync_generation), 0) FROM events`, &sqlitex.ExecOptions{
			ResultFunc: func(stmt *sqlite.Stmt) error {
...
func (tx *Tx) UpsertEvent(e calendar.Event, syncGen int64) error {
	return upsertEvent(tx.conn, e, syncGen)
}

func (tx *Tx) SetSyncToken(calendarID, token string) error {
	return setSyncToken(tx.conn, calendarID, token)
}

func (d *DB) WithTransaction(ctx context.Context, fn func(tx *Tx) error) (err error) {
	conn, err := d.pool.Take(ctx)
	if err != nil {
		return fmt.Errorf("take transaction connection: %w", err)
	}
	defer d.pool.Put(conn)
