...
3. **`calvin week` command** — Show the next 7 days of events, not just today. (Or `calvin events --range=week`.) ✅ Done in `83a7c27`
...
7. **`calvin free` command** — Show free/busy gaps between today's meetings. Pipe-friendly for scheduling scripts. ✅ Done in `5cad817`
...
10. **All-day event support** — `convertEvent` currently skips all-day events. Fall back to `item.Start.Date` so birthdays, deadlines, and OOO markers can trigger hooks. ✅ Done in `11f287b`
